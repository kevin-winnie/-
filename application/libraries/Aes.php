    ...
    */
    public static function AesEncrypt($plaintext,$key = null)
    {
        ...
        if(!extension_loaded('mcrypt')) {
            $error = array('status'=>'fail','errorMessage'=>'AesEncrypt requires PHP mcrypt extension to be loaded in order to use data encryption feature.');
...
{
   // $hashData = bin2hex(mhash(MHASH_SHA256,$data.HASH256_KEY));
      $hashData = hash('sha256',trim($data) . HASH256_KEY);
     //测试 $hashData = hash('sha256',trim($data) . OMSTESTHASH256_KEY);
    return $hashData;
}
